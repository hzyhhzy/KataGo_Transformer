...
Training stores `binaryInputNCHW` in NHWC memory format by default, for both
masked and maskless runs. Parameters, optimizer tensors, value targets,
ordinary validation, and SWA validation retain their existing layouts.
...
NHWC storage: stride (57600, 256, 1)
```
...
contiguous. NHWC allows RMSNorm, QKV projections, and FFN projections
...
```text
-input-memory-format nhwc -> NHWC (default)
-input-memory-format nchw -> NCHW
```
...
as an NHWC view. The legacy `TransformerBlock` also passes
`key_padding_mask=None` when masking is disabled.

### SDPA backend control

`-sdpa-backend auto|flash|cudnn|efficient|math` can force one backend for
...
batch-432 attempt ran out of memory by approximately 72 MiB; NHWC batch
432 was not tested.
...
maskless NHWC training. All runs use the same filtered full15 data,
...
| b24 | optimized, maskless NHWC | 384 | 22.240 / 22.130 s | 1.36% | 3,453.2 | +50.43% |
...
| b40 | optimized, maskless NHWC | 160 | 27.983 / 28.000 s | 0.99% | 1,143.5 | +50.20% |

At a fixed batch, the maskless NHWC path is another 25.25% faster
...
The final maskless NHWC path was repeated without `-multi-gpus`,
...
- A small complete Transformer model matched between NCHW and NHWC for
  outputs, input gradients, global-input gradients, and parameter gradients,
  with both masked and maskless forwards.
- The optional attention-pool path matched for NCHW and NHWC outputs
  and gradients.
...
- A final two-GPU smoke test confirmed that NHWC is selected by default.
...
`-input-memory-format nchw` only for regression comparison or if a previously
